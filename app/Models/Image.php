...
class Image extends Model{
    ...
    protected function saveThumbs($data,$thumbs){
        foreach($thumbs as $item){
            $this->thumbnail($data->getPathName(),$this->path . DIRECTORY_SEPARATOR . $item[2].$this->filename,$width = $item[0],$height = $item[1],$quality = 75); //пропорционально
        }
    }
    ...
    public function saveOriginal($data){
        if(move_uploaded_file($data->getPathName(), $this->path . DIRECTORY_SEPARATOR . $this->filename)){
            return true;
        }
        return false;
    }
    ...
    protected function validateFileSize($data){
        ...
        if($data->getClientSize() > $filesize){
            $this->validation = false;
